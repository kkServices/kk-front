import { definePreset, escapeRegExp } from 'unocss'
...
const presetChroma = definePreset((options: ChromaOptions = {}) => {
      ...
      [
        ...
        ([, type = 'linear', precision, mode, body], { theme }) => {
          ...
          if (Number.isNaN(steps) || steps < 1)
            steps = defaultSteps
          const stops = resolveStops(steps, mode as InterpolationMode, body, theme)
          if (stops != null) {
            return {
              '--un-gradient-shape': defaultShapes[type as keyof typeof defaultShapes],
...
export { presetChroma }
function resolveStops(steps: number, mode: InterpolationMode, body: string, theme: any) {
  const cs: any[] = []
  while (true) {
  ...
  for (let i = 0; i < s; i++) {
    ...
    const r = resolveChroma(last ? steps : steps + 1, mode, cs[i].color, cs[i + 1].color)
      .map(([c, p]) => [parseColor(c.toString(), theme)?.cssColor?.components?.join(','), Math.floor(i * 100 / s + (p as number) / s)])
      .map(([c, p]) => `rgb(${c}) ${p}%`)
...
function resolveChroma(steps: number, mode: InterpolationMode, start: string, end: string) {
